...
% fill pattern in the X direction (horizontal printing) with the
...
square_margin = 0.04; % cm --> gap --> dependes on the infill (0.025-0.035 cm)
solid_layers = 0.06; %cm --> 0.02 x 3 (top and bottom layers)

infill = input("What is the desired infill percentage?");
number_squares = (infill*13)/15; % 15% corresponds to 13 full squares in both ways
...
if floor(number_squares)==number_squares % full number of squares
    if rem(floor(number_squares),2) == 0 % even number
        ...
        repeater_y_1 = floor(number_squares);
        repeater_y_2 = floor(number_squares) + 1;  % halfs
    else % odd number
        diagonal = round((((10-2*slab_margin)-floor(number_squares)*square_margin)/number_squares), 4);
        repeater_y_1 = floor(number_squares) + 1; % halfs
        repeater_y_2 = floor(number_squares);
    end
else % not a full number of hexagons
    if rem(floor(number_squares),2) == 0 % even number
        diagonal = round((((10-2*slab_margin)-(floor(number_squares)+1)*square_margin)/number_squares), 4);
        repeater_y_1 = floor(number_squares) + 2;
        repeater_y_2 = floor(number_squares) + 1;
    else % odd number
        diagonal = round((((10-2*slab_margin)-floor(number_squares)*square_margin)/number_squares), 4);
        repeater_y_1 = floor(number_squares) + 1;
        repeater_y_2 = floor(number_squares) + 2;
    end
end
...
fid = fopen('main_grid_x.mac', 'w');

% INTRODUCTION
fprintf(fid, '\n%s \n%s %d\n\n', '# Grid X direction', '# Infill percentage:', infill);
...
fprintf(fid, '\n%s %.2f %s', '/gate/square1/geometry/setXLength', 1-(solid_layers*2), 'cm');
fprintf(fid, '\n%s %.2f %s', '/gate/square1/geometry/setYLength', square_edge, 'cm');
fprintf(fid, '\n%s %.2f %s', '/gate/square1/geometry/setZLength', square_edge, 'cm');
fprintf(fid, '\n/gate/square1/placement/setRotationAxis 1 0 0');
...
fprintf(fid, '\n/gate/slab/daughters/insert box');
fprintf(fid, '\n%s %.2f %s', '/gate/square2/geometry/setXLength', 1-(solid_layers*2), 'cm');
fprintf(fid, '\n%s %.2f %s', '/gate/square2/geometry/setYLength', square_edge, 'cm');
fprintf(fid, '\n%s %.2f %s', '/gate/square2/geometry/setZLength', square_edge, 'cm');
fprintf(fid, '\n/gate/square2/placement/setRotationAxis 1 0 0');
...
fprintf(fid, '\n/gate/square1/cubicArray/setRepeatNumberX 1');
fprintf(fid, '\n%s %.0f', '/gate/square1/cubicArray/setRepeatNumberY', repeater_y_1);
fprintf(fid, '\n%s %.0f', '/gate/square1/cubicArray/setRepeatNumberZ', repeater_y_2);
fprintf(fid, '\n%s %.3f %.3f %s', '/gate/square1/cubicArray/setRepeatVector 0', diagonal+square_margin, diagonal+square_margin, 'cm');

% repeaters 2
fprintf(fid, '\n\n# REPEATER 2');
fprintf(fid, '\n/gate/square2/repeaters/insert cubicArray');
fprintf(fid, '\n/gate/square2/cubicArray/setRepeatNumberX 1');
fprintf(fid, '\n%s %.0f', '/gate/square2/cubicArray/setRepeatNumberY', repeater_y_2);
fprintf(fid, '\n%s %.0f', '/gate/square2/cubicArray/setRepeatNumberZ', repeater_y_1);
fprintf(fid, '\n%s %.3f %.3f %s', '/gate/square2/cubicArray/setRepeatVector 0', diagonal+square_margin, diagonal+square_margin, 'cm');
...
fprintf(fid, '\n/gate/actor/stat/save output/stats_grid_x_%0.0f.txt', infill);
...
fprintf(fid, '\n/gate/actor/EM-properties/save output/EMprops_grid_x_%0.0f.txt', infill);
...
fprintf(fid, '\n/gate/actor/doseDistribution/save output/output_grid_x_%0.0f.mhd', infill);
